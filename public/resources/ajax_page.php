...
if(file_exists($img))
{
	if($ro->image!='')
	{
			$img = BASE_PATH . $ro->path() . webp($ro->image);
	}else
	{
		$img=BASE_PATH.DS.BASE_FOLDER.UPLOAD_PATH. webp('sample.jpg');
		}
}else
{
	$img=BASE_PATH.DS.BASE_FOLDER.UPLOAD_PATH
		. webp('sample.jpg');
	}
...
$nestedData[] =	ucfirst($ro->menus->title);
