...
function generate_menu($menu_items)
{
  ...
  foreach ($menu_items as $item) {
    ...
    $output .= '<li class="menu"><a href="' . $item['link'] . '" '. $extra. '><div class="">' . $item['icon'] . '<span>' . $item['label'] . '</span></div><div>
            <i data-feather="feather-chevron-right"></i>
    ...
    if (isset($item['children']) && is_array($item['children'])) {
      ...
      if (isset($item['children'][0]['child_class'])) {
        $output .= ' class="' . $item['children'][0]['child_class'] . '"';
      }

      if (isset($item['children'][0]['child_id'])) {
        $output .= ' id="' . $item['children'][0]['child_id'] . '"';
      }
      if (isset($item['children'][0]['child_extra'])) {
        $child_extra =  $item['children'][0]['child_extra'];
      } else {
        $child_extra = '';
      }
      $output .= $child_extra .'>';
      ...
      foreach ($item['children'] as $child_item) {
        $output .= '<li><a href="' . $child_item['link'] . '">' . $child_item['label'] . '</a></li>';
      }
