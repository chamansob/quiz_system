...
redirect_by_js('add',1000);
  ...
  public static function clearall($table)
  {
    ...
    $val = $database::select('select 1 from `' . $table . '` LIMIT 1');
    if ($val !== false) {
      echo 'DO SOMETHING! IT EXISTS!<br>';
      $val = $database::select('Drop TABLE `' . $table . '`');
      if ($val) {
        echo "All good";
      }
    } else {
  ...
  {
    $attributes=" ['class' => 'form-control', 'placeholder' => '".ucfirst($n)."'])";
    $x = '$fo=Form::input("' . ucfirst($n) . '","' . $n . '",$' . $n . ' '. $attributes.');';
    return $x;
  }
